  ...
  try {
    ...
    if (routingResult.agent === "weather") {
      ...
      try {
        ...
        if (!city)
          throw new Error(
            'Could not determine city from your message. Try: "weather in Mumbai"',
          );
        ...
        const weatherContext = [
          ...
          weatherData.hourly?.length
            ? `Hourly: ${weatherData.hourly
                .slice(0, 6)
                .map((h: any) => `${h.time} ${h.temp}°C ${h.description}`)
                .join(", ")}`
            : "",
        ]
          .filter(Boolean)
    ...
    if (!conversation.title && conversation.messages.length === 0) {
      const title =
        await geminiService.generateConversationTitle(messageHistory);
