...
function validateEnvironment() {
    ...
    if (!process.env.POSTMARK_SERVER_TOKEN)
        warnings.push('POSTMARK_SERVER_TOKEN (emails will be logged instead)');
    if (!process.env.FROM_EMAIL)
        warnings.push('FROM_EMAIL (required if using Postmark)');
    if (!process.env.NODE_ENV)
        warnings.push('NODE_ENV (defaulting to development mode)');
    if (!process.env.TAVILY_API_KEY)
        warnings.push('TAVILY_API_KEY (web search agent will not work without it)');
    if (missing.length > 0) {
