...
function wmoToWttrCode(wmo) {
    ...
    if (wmo <= 65)
        return 296; // Rain
    if (wmo <= 67)
        return 311; // Freezing rain
    if (wmo <= 77)
        return 326; // Snow
    if (wmo <= 82)
        return 299; // Rain showers
    if (wmo <= 86)
        return 335; // Snow showers
    if (wmo >= 95)
...
async function weatherAgent(city) {
    ...
    const currentHourIdx = hourly.time.findIndex((t) => t === nowISO);
    ...
    const now = new Date();
    const datetime = now.toLocaleDateString("en-US", { month: "short", day: "numeric" }) +
        ", " +
